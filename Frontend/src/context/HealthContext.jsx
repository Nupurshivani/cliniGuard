...
export const HealthProvider = ({ children }) => {
    ...
    const register = async (name, email, password,role) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/register`, {
                method: 'POST',
                headers: {
    ...
    const login = async (email, password) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/login`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const   fetchuser= async()=>{
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/fetchuser`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const botchat = async (id, message) => {
        try {
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/chat`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
       ...
       const aiinfo = async ( message) => {
        try {
          const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/info`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
